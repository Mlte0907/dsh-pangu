...
- **2026-09-26** — 补知识页抽屉的浏览器验收脚本（25 项断言），此前该页零自动化覆盖、改一次只能靠手工点。
  - **背景**：上一轮改完抽屉后我在维护日志里写了「知识页抽屉暂无自动化验收，
    本次数字来自一次性受控实验脚本」—— 那句话是对的，但留着就是个洞。
    用户问「移动端看过了么」之后，把这个洞补上。
  - **加**：`scripts/verify-knowledge.cjs`（25 项断言，`npm run test:browser:knowledge`），
    覆盖六组：A 关闭态（列表占满可用宽、抽屉 opacity 0、无高亮行）、
    B 点行（抽屉打开、列表压窄但 `offsetParent` 非空、恰好 1 个当前行、
    当前行与详情标题一致）、C 键盘 ↑↓（详情换条、当前行跟随、
    **列表 DOM 完全未重建**、仍恰好 1 个当前行）、D Home/End 跳首尾、
    E Esc 收起（列表恢复整宽、可再次打开）、F 全程零 JS 异常。
  - **「列表 DOM 未重建」的判据**：给每行 DOM 打随机身份戳，按键后比对戳集合。
    62/62 个节点身份存活 —— 这是 `React.memo` 真生效的唯一硬证据，
    光看「界面没闪」是证明不了的。
  - **踩**：断言最初写死「列表 ≥900px」，实测 884px 判红。
    根因是**列表宽度由宿主主区决定，不是视口**（1280 视口实测 884px、
    1440 视口 1044px，差值正好是侧栏宽度）。已改成与主容器宽度比对。
    **写布局断言前先想清楚参照系是谁** —— 宿主会吃掉一截宽度。
  - **验证**：`node scripts/verify-knowledge.cjs` → 25/25。
    `PANGU_TEST_MODULES=<repo>/node_modules node --test` → 117/117。
