    ...
    function PanguSettings() {
      ...
      const load = React.useCallback(async () => {
        try {
          ...
          callRemote('panguDashboard', 'data').then(unwrap).then((v) => setStats(v || null)).catch(() => {})
      ...
      return h('div', { style: { padding: '18px 20px 20px', color: css.t1, maxWidth: 720, animation: 'panguFade .25s ease' } },
        ...
        config && h(StatusCard, { cells: [
          ...
          {
            label: '记忆总量', state: stats ? 'known' : 'unknown',
            value: stats ? String(stats.total) : '—',
            detail: stats ? stats.wings + ' 翼 · ' + stats.rooms + ' 房间' : '未取到统计',
          },
          {
        ...
        draft ? [
          ...
          h('div', { key: 's3-body', style: { paddingTop: 2 } },
            ...
            h(InfoRow, { label: '记忆总量', value: stats ? stats.total + ' 条' : '(未取到)' }),
