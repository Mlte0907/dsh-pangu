...
.pangu-dashboard-grid>*{min-width:0}
.pangu-dashboard-scroll{flex:1 1 auto;min-height:0;overflow:auto!important;height:auto!important;box-sizing:border-box;scrollbar-gutter:stable;overscroll-behavior:contain}
...
.pangu-dashboard-list-col .pangu-dashboard-scroll{max-height:none!important;flex:1 1 auto;min-height:0;overscroll-behavior:contain}
...
`
    ...
    function CrystalPane({ nodes, edges, rawCount, search, onSearch, typeFilter, onTypeFilter, kgErr, loading, onRetry }) {
      ...
      return h('div', { className: 'pangu-dashboard-scroll', style: { padding: '22px 26px 36px' } },
        h('section', { className: 'pangu-dashboard-view' },
          ...
          h('div', { className: 'pangu-dashboard-foot' },
            (rawCount != null && rawCount !== nodes.length
              ? '图谱接口本次返回 ' + rawCount + ' 条实体，按 id 去重后为 ' + nodes.length + ' 条（同名且记忆数/关系数一致的重复项已合并）。'
              : '') +
      ...
      return h('div', { className: 'pangu-dashboard-scroll', style: { padding: '22px 26px 36px' } },
        h('section', { className: 'pangu-dashboard-view' },
          ...
          loading
            ...
            : shown.length
              ? h('div', { className: 'pangu-dashboard-split' },
                  h('div', { className: 'pangu-dashboard-list-col' },
                    h('div', { className: 'pangu-dashboard-list-head' }, h('span', null, '知识条目'), h('span', { style: { marginLeft: 'auto' } }, String(shown.length) + ' 条')),
                    h('div', { className: 'pangu-dashboard-scroll' },
                      shown.map((entry) => {
                        const color = CAT_COLORS[entry.category] || 'var(--v3-ink-3)'
                        return h('button', {
                          key: entry.id, className: 'pangu-dashboard-row', 'data-selected': selectedId === entry.id ? 'true' : undefined,
                          onClick: () => setSelectedId(entry.id), type: 'button',
                    ...
                    selected
                      ? h(React.Fragment, null,
                          ...
                          h('div', { style: { display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', marginTop: 16, fontSize: 12, color: 'var(--v3-ink-3)' } },
                            h('span', null, '更新于 ', h('b', { style: { color: 'var(--v3-ink-1)', fontWeight: 640 } }, updateDate(selected))),
                          ),
                          h('div', { className: 'pangu-dashboard-tip' }, h('b', null, '来源与版本'), h('br'), '当前详情完全来自远程知识库返回字段，不补写或生成额外内容。'),
                        )
                      : h('div', { className: 'pangu-dashboard-empty' }, h(Icon, { name: 'layers', size: 26 }), h('strong', null, '选择一条知识'), h('span', null, '点击左侧条目查看正文、来源数量、标签和使用次数。')),
                  ),
                )
          ...
          h('div', { className: 'pangu-dashboard-foot' }, '分类计数与条目正文均来自 panguKnowledge.stats / list / search；左栏独立滚动，页面高度与知识总量解耦。'),
        ),
      )
    }
    ...
    function PanguTab() {
      ...
      /* 图谱接口当前会把同一个实体返回两遍（实测：HTTP / MCP / REST 等成对出现，
         两行的名称、类型、记忆数、关系数完全一致）。不兜住会有两个后果：
         ① React 列表 key 冲突，翻页时旧行不会被移除，行数逐页累加；
         ② 星系图的 nm/adj 用 Map 建索引，重复项被静默吞掉，
            计数却仍按原始条数显示，对不上。
         这里按 id 去重（保留 memory_count 较大的那条），边同理按「源|目标|关系」去重。 */
      ...
      const pane = tab === 'overview'
        ...
        : tab === 'graph'
          ...
          : tab === 'crystal'
            ? h(CrystalPane, { nodes: filteredNodes, edges: graphEdges, rawCount: (kg?.nodes || []).length, search, onSearch: setSearch, typeFilter, onTypeFilter: setTypeFilter, kgErr: kgError, loading, onRetry: () => load() })
            : tab === 'knowledge'
