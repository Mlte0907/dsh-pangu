 ...
 * v0.6: MCP url/凭据改由 lib/mcp-inject.js 在激活/保存时 entry.update 注入 ——
 *       根治 !!js 求值域无 require 的静默回退(见 dsh-pangu-mcp-disconnect-report.md)。
...
const { injectMcpConfig, installMcpConfigReloadHook, trace: panguTrace } = require('./mcp-inject')

/** 正式日志:console(→web.log)+ 文件 trace + ctx.logger(GUI 日志面板)三通道。 */
function makePlog(ctx) {
  const fmt = (args) => args.map((x) => typeof x === 'string' ? x : String(x)).join(' ')
  return {
    log: (...args) => { const m = fmt(args); console.log(m); panguTrace(m); try { ctx.logger.info(m) } catch (_) {} },
    error: (...args) => { const m = fmt(args); console.error(m); panguTrace(m); try { ctx.logger.error(m) } catch (_) {} },
  }
}
...
async function apply(ctx) {
  // 活性守卫:teardown 后的定时器一律静默(help/短命进程零噪声)。
  let panguAlive = true
  ctx.effect(() => () => { panguAlive = false }, 'dsh-pangu: timer guard')
  const plog = makePlog(ctx)
  // MCP 注入(A2):读 ~/.pangu/config.json → entry.update 注进 mcp-pangu 行。
  // 查找走 ctx.loader.entries() 递归(bundle patch 行在 Include 子树,本层
  // store 查不到 —— 0.6.0/0.6.1 曾因此 miss ×2);1.5s 后再补一枪兜底时序
  // (幂等:配置未变时 update 比对无差异直接返回)。失败仅 console.error,
  // 仪表盘/设置页必须活着(修复入口),配置坏时 mcp 行以
  // failOnStartupError=true 响亮失败。
  try { await injectMcpConfig(ctx, plog) } catch (e) {
    plog.error('[dsh-pangu] MCP injection failed: ' + (e && e.message))
  }
  // profile/Include 重组会先用文件中的占位配置重建 mcp-pangu 行；重组完成
  // 后再把运行时地址和凭据注回，避免行保持 ACTIVE 却连接占位地址。
  installMcpConfigReloadHook(ctx, plog)
  setTimeout(() => {
    if (!panguAlive) return
    injectMcpConfig(ctx, plog).catch((e) => plog.error('[dsh-pangu] MCP delayed re-injection failed: ' + (e && e.message)))
  }, 1500)

  // —— 运行态 census 探针(0.6.4):一个输出同时裁决 连接/注册/可见性 ——
  // view=tools.get 视图命中;rawPangu=原始注册表 pangu 工具数;fiber= mcp 行状态
  // (0 PENDING/1 LOADING/2 ACTIVE/3 FAILED/4 DISPOSED/5 UNLOADING)。
  const census = (tag) => {
    try {
      const svc = ctx.get && ctx.get('tools')
      let viewHit = 0
      let rawHit = 0
      let total = 0
      if (svc) {
        for (const n of ['mcp__pangu__pangu_stats', 'mcp__pangu__pangu_search_memories']) {
          try { if (svc.get(n)) viewHit++ } catch (_) {}
        }
        if (svc.tools && typeof svc.tools.keys === 'function') {
          for (const k of svc.tools.keys()) { total++; if (k.startsWith('mcp__pangu__')) rawHit++ }
        }
      }
      let row = null
      for (const e of ctx.loader.entries()) { if (e.options && e.options.id === 'mcp-pangu') { row = e; break } }
      const states = ['PENDING', 'LOADING', 'ACTIVE', 'FAILED', 'DISPOSED', 'UNLOADING']
      const st = row && row.fiber ? row.fiber.state : -1
      const stName = !row ? 'NO_ROW' : (st >= 0 ? (states[st] || String(st)) : 'NO_FIBER')
      plog.log(`[dsh-pangu] census(${tag}): view=${viewHit}/2 rawPangu=${rawHit} totalTools=${total} mcpRowFiber=${stName}`)
    } catch (e) { if (panguAlive) plog.error('[dsh-pangu] census failed: ' + (e && e.message)) }
  }
  setTimeout(() => { if (panguAlive) census('4s') }, 4000)
  setTimeout(() => { if (panguAlive) census('12s') }, 12000)
  // 时间轴 census(0.6.6):15s×40,观察 view 是否随连接掉线/给-up 而归零。
  // 短命进程(help 模式)活不到 15s,首个 tick 前即退,不产生噪声。
  let tick = 0
  const timeline = setInterval(() => {
    if (!panguAlive) { clearInterval(timeline); return }
    tick += 1
    census('t' + tick)
    if (tick >= 40) clearInterval(timeline)
  }, 15000)
  try {
    let changes = 0
    ctx.on('tools/change', () => {
      changes += 1
      if (changes <= 5) census('change#' + changes)
    })
  } catch (_) {}
  ...
  async function fetchPanguStats() {
    ...
    try {
      ...
      if (parsed) {
        ...
        llmDaily = parsed?.llm_daily
        llmTotal = parsed?.llm_total
      }
    } catch (e) {
    ...
    // 7 日记忆脉搏：优先用服务端随 stats 下发的按天聚合（2026-09-23 起，pangu
    // collect_stats 的 daily_creates；盘古上云后跨机读服务端文件必然失败，这是
    // 唯一可靠来源）。旧服务端没有该字段时回退本机 drawers.json —— 仅在盘古与
    // DSH 同机部署时可用（历史路径，保留兼容）。
    let dailyCounts = []
    try {
      const serverDaily = parsed?.daily_creates
      if (Array.isArray(serverDaily) && serverDaily.length > 0) {
        dailyCounts = serverDaily
          .filter((x) => x && typeof x.date === 'string')
          .map((x) => ({ date: x.date, count: Number(x.count) || 0 }))
      } else {
    ...
    return { ok: true, total, wings, rooms, kgEntities, kgRelations, byWing, byClass, pipeline, highClass, health, healthScore, version, uptimeSeconds, dailyCounts, dailyRecalls, knowledge, snapshots, bySource, platformsCount, llmDaily, llmTotal, ts: Date.now() }
  }
  ...
  /**
   * 保证 ~/.pangu 存在（0600 之外的目录权限用 0700）。
   *
   * 为什么必须有：写配置走的是"写临时文件 + rename"，目录不存在时 writeFile
   * 直接 ENOENT，用户看到的是
   *   Error: ENOENT: no such file or directory, open '/home/xiaoxin/.pangu/config.json.tmp-1606823'
   * —— 一串看不懂的路径，且完全不知道那个目录是哪来的、该不该自己建
   * （2026-09-22 实测：本机卸载盘古删掉 ~/.pangu 后，设置页点保存必报这个）。
   *
   * 注意：插件**可以只连远程服务端**（服务不在本机），所以缺目录不是"盘古没装"
   * 的错误，而是插件自己的配置目录，缺了就该建。
   */
  async function ensurePanguDir() {
    await fsp.mkdir(path.dirname(CONFIG_PATH), { recursive: true, mode: 0o700 })
  }

  async function saveConfig(patch) {
    try {
      await ensurePanguDir()
      const cfg = await readConfig()
      Object.assign(cfg, patch)
      const tmpFile = CONFIG_PATH + '.tmp-' + process.pid
      // mode 0600：config.json 里存着 api_key（数据面凭据），不该被同机其他用户读走。
      // 经 rename 后目标文件沿用这个权限。
      await fsp.writeFile(tmpFile, JSON.stringify(cfg, null, 2), { mode: 0o600 })
  ...
  const dashboardService = {
    ...
    async checkUpdate() {
      try {
        ...
        // ── 本机实际在跑的版本（2026-09-22 加）──
        // 只报"上游最新"是没有用的：用户无从判断**自己这台**落后没有。这正是
        // "普通用户不知道自己有洞、也不知道该升级"的解药 —— 把两边版本号摆在
        // 一起，落后就明确指出并给出**真能跑通**的升级命令。
        const out = {
          ok: true,
          tag: data.tag_name || '',
          ...
          serverBehind: false,
          pluginBehind: false,
          // 升级命令必须**不依赖 git**：云端最小镜像常没有 git，而 install.sh
          // 自己会取代码（tarball 兜底）。旧的 `git pull && pnpm install` 对
          // 这类部署根本跑不通，还会把人带沟里。
          serverUpgradeCmd: 'curl -fsSL https://raw.githubusercontent.com/Mlte0907/pangu/master/install.sh | bash',
          pluginUpgradeCmd: 'curl -fsSL https://raw.githubusercontent.com/Mlte0907/dsh-pangu/main/install.sh | bash',
        }

        // 服务端版本：/health 自带。取不到就留空 —— 不能把"查不到"说成"已是最新"
        try {
          const h = await fetchJson(`${PANGU_BASE}/health`)
          out.serverVersion = String((h && h.data && h.data.version) || '')
        } catch (_) {}

        // 插件上游版本：raw package.json。先直连，国内不通再走代理；
        // 失败不影响主结果（用户仍能拿到盘古侧的升级信息）
        const pluginMetaUrl = 'https://raw.githubusercontent.com/Mlte0907/dsh-pangu/main/package.json'
        try {
          const pr = await fetch(pluginMetaUrl, { signal: AbortSignal.timeout(5000) })
          if (pr.ok) out.pluginLatest = String((await pr.json()).version || '')
        } catch (_) {
          try {
            const pr2 = await fetch(proxy + pluginMetaUrl, { signal: AbortSignal.timeout(8000) })
            if (pr2.ok) out.pluginLatest = String((await pr2.json()).version || '')
          } catch (_) {}
        }

        const latest = String(out.tag || '').replace(/^v/, '')
        out.serverBehind = !!(out.serverVersion && isNewer(latest, out.serverVersion))
        out.pluginBehind = !!(out.pluginVersion && out.pluginLatest && isNewer(out.pluginLatest, out.pluginVersion))
        return out
      } catch (e) {
  ...
  const configService = {
    async get() {
      ...
      // 服务端是否**真的连得上**：`live` 来自对服务端的一次真实调用
      // （pangu_config_get），失败即 null。
      //
      // 设置页那颗状态灯必须用它，而不是"本地配置读到了就算就绪"——
      // 此前徽标只要 config 是对象就画绿灯，于是**盘古根本没装也显示
      // 「MCP 服务已就绪」**（2026-09-22 用户实测：装完插件、盘古还没装，
      // 页面就报就绪）。状态灯说谎比没有状态灯更糟，它会让人跳过真正的问题排查。
      const serverOnline = live !== null
      // 与 api_key_set / *_hint 同一类「派生状态」，一并挂在 config 载荷上 ——
      // 前端因此无需额外取值路径即可读到（保持与既有约定一致）。
      const payload = redactConfig(config)
      payload.server_online = serverOnline
      payload.server_url = PANGU_BASE
      return { ok: true, config: payload, versions: await localVersions() }
    },
    async save(args) {
      ...
      // 此键是插件本地设置（MCP 客户端经 lib/mcp-inject.js 注入此值），不推给盘古服务。
      ...
      if (res.ok) {
        ...
        // MCP 同步重注(A2):地址或凭据变了 → entry.update 热挂载 mcp-pangu,
        // 保存即生效无需重启;失败仅 console.error,不让设置保存失败。
        if (clean.pangu_base_url !== undefined || clean.api_key !== undefined) {
          try { await injectMcpConfig(ctx, makePlog(ctx)) } catch (e) { console.error('[dsh-pangu] save-time MCP re-injection failed:', e && e.message); try { ctx.logger.error('save-time MCP re-injection failed: ' + (e && e.message)) } catch (_) {} }
        }
