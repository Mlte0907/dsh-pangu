...
const { injectMcpConfig } = require('./mcp-inject')
...
async function apply(ctx) {
  ...
  try { await injectMcpConfig(ctx, console) } catch (e) { console.error('[dsh-pangu] MCP injection failed:', e && e.message) }
  setTimeout(() => {
  ...
  const census = (tag) => {
    try {
      ...
      console.log(`[dsh-pangu] census(${tag}): view=${viewHit}/2 rawPangu=${rawHit} totalTools=${total} mcpRowFiber=${stName}`)
