...
async function apply(ctx) {
  ...
  try { await injectMcpConfig(ctx, console) } catch (e) {
    const failLine = '[dsh-pangu] MCP injection failed: ' + (e && e.message)
    console.error(failLine)
    panguTrace(failLine)
  }
  setTimeout(() => {
    injectMcpConfig(ctx, console).catch((e) => console.error('[dsh-pangu] MCP delayed re-injection failed:', e && e.message))
  }, 1500)
  ...
  const census = (tag) => {
    try {
      ...
      const censusLine = `[dsh-pangu] census(${tag}): view=${viewHit}/2 rawPangu=${rawHit} totalTools=${total} mcpRowFiber=${stName}`
      console.log(censusLine)
      panguTrace(censusLine)
    } catch (e) { console.error('[dsh-pangu] census failed:', e && e.message) }
  }
  setTimeout(() => census('4s'), 4000)
  setTimeout(() => census('12s'), 12000)
    ...
    async save(args) {
      ...
      if (res.ok) {
        ...
        if (clean.pangu_base_url !== undefined || clean.api_key !== undefined) {
          try { await injectMcpConfig(ctx, console) } catch (e) { console.error('[dsh-pangu] save-time MCP re-injection failed:', e && e.message) }
        }
