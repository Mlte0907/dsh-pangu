...
const TRACE_PATH = path.join(os.homedir(), '.dsh', 'pangu-mcp-trace.log')

/**
 * 文件通道留痕(headless 的 stdout 不回显,web.log 只有 console)。
...
function trace(line) {
  try {
    try {
      const st = fs.statSync(TRACE_PATH)
      if (st.size > TRACE_MAX_BYTES) fs.renameSync(TRACE_PATH, TRACE_PATH + '.old')
    } catch (_) { /* stat/rename 失败不阻塞写入 */ }
    fs.appendFileSync(TRACE_PATH, `${new Date().toISOString()} pid=${process.pid} ${line}` + String.fromCharCode(10))
  } catch (_) {}
}
...
function buildRowConfig(resolved) {
  if (resolved.ok) {
    return {
      ...
      // 默认 maxAttempts=10(≈5 分钟)后 give-up 会注销全部工具且不再自愈;
      // 拉到百万级 = 实际上永不放弃,长驻实例掉线后持续重连。
      reconnect: { enabled: true, maxAttempts: 1000000 },
    }
  }
  return {
    serverName: 'pangu',
...
async function injectMcpConfig(ctx, log = console, configPath = DEFAULT_CONFIG_PATH) {
  ...
  if (!entry) {
    const missLine = `[dsh-pangu] row ${MCP_ROW_ID} not found (bundle patch not mounted?), MCP injection skipped`
    log.error(missLine)
    trace(missLine)
    return resolved
  }
  await entry.update({ config: buildRowConfig(resolved) })
  // 成功也留痕(仅 url + key 长度,不泄 key):web.log 是 console 的 stdout 通道。
  const line = `[dsh-pangu] MCP injected: ${resolved.ok ? resolved.url : 'fail-loud default (config bad)'} keylen=${resolved.ok ? resolved.key.length : 0}`
  if (typeof log.log === 'function') log.log(line)
  else if (typeof log.error === 'function') log.error(line)
  trace(line)
  return resolved
}

/**
 * 在 app-boot 完成 profile/Include 重组后重新注回 MCP 配置。
 * 该事件发生在 Loader await 之后，避免和正在进行的行更新互相覆盖。
 * @param {object} ctx 插件上下文。
 * @param {{error: Function}} log 日志对象。
 * @param {string} [configPath] 配置文件路径(测试可注入)。
 * @returns {void}
 */
function installMcpConfigReloadHook(ctx, log = console, configPath = DEFAULT_CONFIG_PATH) {
  if (!ctx || typeof ctx.on !== 'function') return
  let pending = Promise.resolve()
  ctx.on('app-boot/config-reload', () => {
    pending = pending
      .then(() => injectMcpConfig(ctx, log, configPath))
      .catch((error) => {
        log.error(`[dsh-pangu] MCP reload re-injection failed: ${error && error.message ? error.message : String(error)}`)
      })
    return pending
  })
}

module.exports = { MCP_ROW_ID, DEFAULT_URL, DEFAULT_CONFIG_PATH, TRACE_PATH, trace, resolveMcpConfig, buildRowConfig, injectMcpConfig, installMcpConfigReloadHook }
